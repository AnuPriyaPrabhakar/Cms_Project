...
function CustomerEdit() {
    ...
    const handleSearch = () => {
        ...
        customerApiService
            .getManager(startDateAsDate, endDateAsDate)
    ...
    const handleTableRowClick = (cmsId: string, recordTypeId: number) => {
        let route: string;
        switch (recordTypeId) {
            case 1:
                route = 'ManagerEntityview';
                break;
            case 2:
                route = 'ManagerIndividualview';
                break;
            case 3:
                route = 'ManagerShipview';
                break;
            case 4:
                route = 'ManagerAircraftview';
                break;
            default:
                route = '';
        }
        navigate(`/View/${route}/${cmsId}/${loginDetails.id}/${recordTypeId}`);
    };
    ...
    return (
        <>
            <Box sx={{ display: 'flex' }}>
                <Header />
                <Box m={4}>
                    <Container style={{ maxWidth: 'none', backgroundColor: 'white', padding: "30px", margin: "5px", position: 'fixed' }}>
                        <Box m={4}>
                            <h3>Manager</h3>
                                                    ...
                                                    <TableBody>
                                                        {data.filter(item => selectedadminuser === '' || item.userName.toLowerCase() === selectedadminuser.toLowerCase())
                                                            .map((item: CustomerPayload, index: number) => {
                                                                console.log("created_at value:", item.created_at); // Add this line for debugging
                                                                return (
                                                                    <TableRow key={index}>
